...
export function LabelsClient({ roasterName, batches, currency }: Props) {
  ...
  return (<div>
      <div className="page-header">
        ...
        {selectedBatch && (<button onClick={handlePrint} className="btn-primary">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 flex flex-col gap-5 no-print">
          ...
          <div className="card p-5">
            ...
            >
              ...
              {batches.map((b: RoastBatch) => (<option key={b.id} value={b.id}>
          ...
          <div className="card p-5 flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {SIZE_OPTIONS.map(s => (<button key={s.id} type="button"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {WEIGHT_OPTIONS.map(w => (<button key={w} type="button"
                    onClick={() => setWeight(w)}
          ...
          <div className="card p-5 flex flex-col gap-3">
            ...
            ].map(({ key, label, value, set }) => (<label key={key} className="flex items-center justify-between cursor-pointer">
        ...
        <div className="lg:col-span-2">
          {!selectedBatch ? (<div className="card flex items-center justify-center" style={{ minHeight: 300 }}>
  ...
  return (<div
    ...
    >
      ...
      <div style={{ paddingLeft: forPrint ?"3mm" : 10, flex: 1, display:"flex", flexDirection:"column", gap: forPrint ?"1mm" : 4 }}>
        ...
        {/* Origen */}
        {showOrigin && coffee?.origin_country && (<p style={{ fontSize: forPrint ?"7pt" : 10, color: isDark ?"#E8D5C4" :"#6B5744" }}>
            {coffee.origin_country}
            {coffee.farm_producer ? ` · ${coffee.farm_producer}` :""}
          </p>)}

        {/* Variedad y proceso */}
        {(coffee?.variety || coffee?.process) && (<p style={{ fontSize: forPrint ?"6.5pt" : 9, color: isDark ?"#BBA99A" :"#6B5744" }}>
            {[coffee.variety, coffee.process].filter(Boolean).join(" ·")}
          </p>)}

        {/* Nivel de tueste */}
        {batch.roast_level && (<p style={{
        ...
        {/* Puntaje */}
        {showScore && coffee?.score && (<p style={{ fontSize: forPrint ?"6pt" : 9, color: isDark ?"#BBA99A" :"#6B5744" }}>
            Q Score: {coffee.score}
          </p>)}

        {/* Notas de cata */}
        {showTasting && coffee?.tasting_notes && (<p style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          {/* Fecha */}
          {showRoastDate && (<p style={{ fontSize: forPrint ?"5.5pt" : 8, color: isDark ?"#BBA99A" :"#6B5744" }}>
              Tostado: {formatDate(batch.roast_date)}
            </p>)}

          {/* Tagline */}
          {customTagline && (<p style={{ fontSize: forPrint ?"5.5pt" : 8, color: isDark ?"#BBA99A" :"#6B5744", marginTop: 2 }}>
              {customTagline}
            </p>)}
        </div>

        {/* QR */}
        {showQR && (<div style={{ flexShrink: 0 }}>
            <QRCodeSVG
              value={qrValue}
              size={forPrint ? 28 : 44}
              bgColor="transparent"
