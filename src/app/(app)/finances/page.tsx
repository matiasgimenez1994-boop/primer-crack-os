...
export default async function FinancesPage() {
  ...
  const totalPending = (pendingSales ?? []).reduce(
    (s: number, x: Sale) => s + (x.final_price - x.amount_paid), 0
  );

  // â”€â”€ Inventario valorizado â”€â”€
  const inventoryValue = (greenCoffees ?? []).reduce(
    (s: number, c: { current_stock_kg: number; purchase_price_per_kg: number }) =>
      ...
      <div className="page-header">
        ...
        <div className="flex gap-2">
          ...
          {(pendingSales ?? []).length > 0 && (
            <Link href="/finances/pending" className="btn-primary text-xs">
        ...
        <div className="flex items-end gap-3 h-40">
          {monthlyData.map((m) => (
            <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
              ...
              {m.revenue > 0 && (
                <p className="text-xs font-mono text-text-primary">{formatCurrency(m.revenue, roaster.currency)}</p>
              )}
            </div>
          ))}
        </div>
