...
export function ReportsClient({ data }: { data: ReportData }) {
  ...
  function handleExportCSVVentas() {
    const csv = generateCSV(
      ["Fecha", "Cliente", "Producto", "Tipo", "Cantidad", "Precio", "Descuento%", "Total", "Ganancia", "Pago", "Estado"],
            ...
            <div className="flex items-end gap-3 h-40">
              {data.monthlyRoasting.map((m) => (
                <div key={m.month} className="flex-1 flex flex-col items-center gap-2">
                  ...
                  {m.lotes > 0 && (
                    <p className="text-xs text-text-secondary">{m.lotes} lotes</p>
                  )}
                </div>
              ))}
            </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.topRoasted.map((c, i) => {
                  ...
                  return (
                    <tr key={c.name} className="border-b border-border-default last:border-0">
      ...
      {activeTab === "sales" && (
        <div className="card overflow-hidden">
          ...
          {data.allSales.length === 0 ? (
            <p className="text-sm text-text-secondary p-6">No hay ventas registradas</p>
          ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.allSales.map((s, i) => (
                    <tr key={i} className="border-b border-border-default last:border-0 hover:bg-[#F5EFE6]/40">
