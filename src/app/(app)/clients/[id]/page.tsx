          ...
          <div className="card p-5">
            ...
            <div className="flex flex-col gap-2.5">
              ...
              {!client.email && !client.phone && (
                <p className="text-sm text-text-secondary">Sin datos de contacto</p>
              )}
              {client.notes && (
                <p className="text-xs text-text-secondary mt-1 pt-2 border-t border-border-default">{client.notes}</p>
              )}
            </div>
          </div>
                  ...
                  <tbody>
                    {allSales.map((s: Sale) => {
                      const name = s.product_type === "roasted"
                        ...
                        : `${(s as any).green_coffees?.name ?? "Verde"} (verde)`;
                      return (
                        <tr key={s.id} className="border-b border-border-default last:border-0 hover:bg-[#F5EFE6]/50">
                          ...
                          <td className="px-5 py-3 text-right font-mono font-medium">
                            ...
                            {s.discount_pct > 0 && (
                              <span className="ml-1 text-xs text-status-warning">(-{s.discount_pct.toFixed(0)}%)</span>
                            )}
                          </td>
