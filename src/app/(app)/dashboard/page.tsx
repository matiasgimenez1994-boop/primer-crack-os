...
export default async function DashboardPage() {
  ...
  const totalGreenStock = (coffees ?? []).reduce(
    (sum: number, c: GreenCoffee) => sum + c.current_stock_kg,
    0
  );

  const inventoryValue = (coffees ?? []).reduce(
    (sum: number, c: GreenCoffee) =>
      sum + c.current_stock_kg * c.purchase_price_per_kg,
    0
  );

  const kgRoastedMonth = (monthBatches ?? []).reduce(
    (sum: number, b: { roasted_weight_kg: number }) =>
    ...
    (recentBatches ?? []).length > 0
      ? (recentBatches ?? []).reduce((sum: number, b: RoastBatch) => {
          ...
          const m = calculateMargin(
            b.total_cost_per_kg_roasted * 1.4,
            1000,
      ...
      {lowStockCoffees.length > 0 && (
        <div className="mb-6 bg-orange-50 border border-orange-200 rounded-xl p-4">
          ...
          <div className="flex flex-col gap-1">
            {lowStockCoffees.map((c: GreenCoffee) => (
              <Link
                key={c.id}
                href={`/inventory/${c.id}`}
        ...
        <div className="mb-4 bg-blue-50 border border-blue-200 rounded-xl p-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            {inactiveClients.length > 3 && (
              <Link href="/clients" className="text-xs text-blue-600 hover:underline mt-1">
              ...
              return (
                <Link key={o.id} href={`/orders/${o.id}`}
                  ...
                  <div>
                    ...
                    {o.delivery_date && (
                      <p className={`text-xs mt-0.5 ${isOverdue ? "text-status-danger font-medium" : "text-text-secondary"}`}>
        ...
        {(recentBatches ?? []).length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {(recentBatches ?? []).map((b: RoastBatch) => (
                  <tr
                    key={b.id}
        ...
        <div className="card overflow-hidden mt-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {(coffees ?? []).slice(0, 5).map((c: GreenCoffee) => (
                  <tr
                    key={c.id}
